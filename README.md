...
- 微信支付已接入小程序 JSAPI/API v3；提现、退款、佣金结算仍不在本服务直接执行。

## 订单验收基线

订单流程会先进入“待客服联系确认”，客服通过 `accept` 操作保存确认服务快照后才开放定金；线下选片确认后才开放尾款。小程序支付由服务端创建 API v3 预支付单并在验签回调或查单确认后落账；售后处理中会冻结新的支付发起。人工收款登记仍需填写外部流水号或人工核对编号。

微信支付的凭据、数据库迁移、回调地址和验收流程见 [微信支付接入说明](docs/微信支付接入说明.md)。

微信支付审核期间可在部署环境的 `.env` 设置 `PAYMENT_MODE=mock` 后重启服务。客户点击现有支付按钮会由服务端完成订单归属、金额、售后和阶段校验，并写入 `provider=mock_payment`、`mock:` 外部流水号、订单支付确认字段和审计日志；不会调用微信支付，也不会把模拟流水伪装成微信支付。审核通过并完成商户配置后，将该值改为 `wechat`（或留空）并重启服务即可让后续订单恢复真实 JSAPI 支付，历史模拟流水会保留原始审计标记。
